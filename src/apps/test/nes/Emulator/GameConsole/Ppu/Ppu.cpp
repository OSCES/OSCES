...
void Ppu_t::SpritesRun( void ) 
{
    ...
    //    for( uint16_t xPos = m_LineCycles; xPos < ( m_LineCycles + m_CyclesToExecute ); xPos++ )
    {
        ...
        if( ( 0 != xPos ) && ( xPos <= PPU_HORIZONTAL_RESOLUTION ) )
        {
            ...
            uint8_t nRenderSprite = 0;

            for ( ; nRenderSprite < 8; nRenderSprite++ ) // Поиск номера спрайта, который нужно отобразить
            {
            ...
            if( nRenderSprite != 0xFF )
            {
                if( m_SpriteFifo[nRenderSprite].xPos == xPos )
                {
                    m_SpritePixel = 0;
                }

                uint8_t spriteLineOffset = 7 - m_SpritePixel;
                if( m_SpriteFifo[nRenderSprite].HorMirror )
                    spriteLineOffset = m_SpritePixel;

                uint8_t spritePixelColor  = ( m_SpriteFifo[nRenderSprite].LowTile >> spriteLineOffset ) & 0x01;
                spritePixelColor |= ( ( m_SpriteFifo[nRenderSprite].HighTile >> spriteLineOffset ) & 0x01) << 1;

                if (spritePixelColor > 0)
                {
                    spritePixelColor |= m_SpriteFifo[nRenderSprite].Color << 2;

                    m_SpriteScanLine[xPos] = spritePixelColor;

                    if( 0 == m_SpriteFifo[nRenderSprite].Priority )
                    {
        ...
        else if( xPos <= PPU_SPRITE_EVALUATION_END_XPOS ) // Sprites evaluation
        {
            ...
            if( ( 0 == xPos % 3 ) && ( PPU_MAX_NUMBER_OF_SPRITES_ON_LINE > m_EvalSpriteIdx ) )
            {
                ...
                if ( ( m_LineCounter >= spriteInfo.yPos ) && ( m_LineCounter < (spriteInfo.yPos + m_SpriteSize) ) ) // Предвыборка спрайтов, которые находятся на следующей скан-линии
                {
        ...
        else if( xPos <= PPU_SPRITE_FETCH_END_XPOS )// Копируем из Secondary OAM в FIFO
        {
            ...
            if( 0 == xPos % 8 )
            {
                ...
                uint8_t yPixelOfSprite = m_LineCounter - spriteInfo.yPos;
